...
CAMBIOS v28 (fix/closed-bar):
  Fix 3 — _compute_indicators: todos los cálculos de serie (EMA, RSI, MACD,
  Supertrend, ATR, vol_avg) se hacen sobre bars[:-1] (velas cerradas).
  La vela activa (bars[-1]) solo se usa para:
    - close: precio de referencia actual
    - price_dir: dirección del último tick
    - vol_ratio: volumen actual vs media de cerradas (comportamiento correcto)
  Esto evita que un spike intracandle flipe ema_bull/ema_bear y genere
  señales falsas con contexto bajista.
