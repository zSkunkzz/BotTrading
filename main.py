...
async def _purge_stale_state() -> set:
    ...
    try:
        ...
        logger.warning(
            "\u26a0\ufe0f  No se pudo consultar BingX: %s — conservando state.", e
        )
        return saved_symbols
    ...
    if stale:
        for sym in stale:
            clear_position(sym)
            logger.warning("\U0001f9f9 STALE STATE: '%s' eliminado.", sym)
        logger.info(
            "\U0001f9f9 State purgado: %d fantasmas (%s). Reales: %s",
            len(stale), ", ".join(stale),
            ", ".join(real_positions) if real_positions else "ninguna",
        )
    else:
        logger.info("\u2705 State OK: %s", ", ".join(saved_symbols))
        ...
        try:
            ...
            for sym, cycles in list(_idle_cycles.items()):
                ...
                if trader is not None and getattr(trader, "_force_idle_rotate", False):
                    logger.info("[%s] \U0001f504 _force_idle_rotate=True — rotando inmediatamente.", sym)
            ...
            for sym in to_rotate:
                ...
                logger.info("\U0001f504 Rotando: %s → %s", sym, new_sym)
...
async def main():
    ...
    try:
        ...
        logger.warning("\u26a0\ufe0f  balance_svc.init_bingx() no disponible — desactivado.")
    except Exception as e:
    ...
    from bot.backtest_scheduler import get_scheduler, set_active_symbols_fn

    _tgbot.set_notifier(_notifier)

    # Inyectar la función que devuelve los pares activos en tiempo real
    set_active_symbols_fn(lambda: list(active_traders.keys()))

    get_scheduler(_notifier).start()
    logger.info(
        "\u23f0 Backtest scheduler activo — analizará los pares activos del bot "
        "cada %s días a las %s:00 UTC",
